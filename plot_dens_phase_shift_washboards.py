...
moms = momfile('mom_i'+suffix,pars)
#print "time0",time0
...
if x_local:
    ...
    #plt.title(r'$\phi$')
    #plt.xlabel(r'$\rho_{tor}$',fontsize=13)
    #plt.plot(xgrid,np.real(phi_bnd[pars['nz0']/2,0,:]),color = 'black')
    #plt.plot(xgrid,np.real(phi_bnd[pars['nz0']/4,0,:]),color = 'red')
    #plt.plot(xgrid,np.real(phi_bnd[3*pars['nz0']/4,0,:]),color = 'blue')
    #plt.plot(xgrid,np.imag(phi_bnd[pars['nz0']/2,0,:]),'--',color = 'black')
    #plt.plot(xgrid,np.imag(phi_bnd[pars['nz0']/4,0,:]),'--',color = 'red')
    ...
    phi_amp = np.abs(phi_bnd)
    phi_phase = np.real(-1J*np.log(phi_bnd/phi_amp))
    dens_amp = np.abs(dens)
    dens_phase = np.real(-1J*np.log(dens/dens_amp))

    plt.figure(figsize=(5,8))
    plt.subplot(2,1,1)
    plt.title(r'$\phi$')
    plt.plot(xgrid,(phi_amp[pars['nz0']/2,0,:]),color = 'black',label='nz/2 amp')
    plt.plot(xgrid,(phi_amp[3*pars['nz0']/4,0,:]),color = 'blue',label='3nz/4 amp')
    plt.plot(xgrid,(phi_amp[pars['nz0']/4,0,:]),color = 'red',label='nz/4 amp')
    for i in range(len(qrats)):
        ix = np.argmin(abs(geometry['q']-qrats[i])) 
    ...
    plt.subplot(2,1,2)
    plt.plot(xgrid,(phi_phase[pars['nz0']/2,0,:]),'--',color = 'black',label='nz/2 phase')
    plt.plot(xgrid,(phi_phase[3*pars['nz0']/4,0,:]),'--',color = 'blue',label='3nz/4 phase')
    plt.plot(xgrid,(phi_phase[pars['nz0']/4,0,:]),'--',color = 'red',label='nz/4 phase')
    plt.xlabel(r'$\rho_{tor}$',fontsize=13)
    for i in range(len(qrats)):
        ix = np.argmin(abs(geometry['q']-qrats[i])) 
        plt.axvline(xgrid[ix],color='black')
    ...
    plt.subplot(2,1,1)
    plt.title('Dens')
    plt.plot(xgrid,(dens_amp[pars['nz0']/2,0,:]),color = 'black',label='nz/2 amp')
    plt.plot(xgrid,(dens_amp[3*pars['nz0']/4,0,:]),color = 'blue',label='3nz/4 amp')
    plt.plot(xgrid,(dens_amp[pars['nz0']/4,0,:]),color = 'red',label='nz/4 amp')
    for i in range(len(qrats)):
        ix = np.argmin(abs(geometry['q']-qrats[i])) 
        plt.axvline(xgrid[ix],color='black')
    plt.xlabel(r'$\rho_{tor}$',fontsize=13)
    plt.legend(loc='lower left')
    plt.subplot(2,1,2)
    plt.plot(xgrid,(dens_phase[pars['nz0']/2,0,:]),'--',color = 'black',label='nz/2 phase')
    plt.plot(xgrid,(dens_phase[3*pars['nz0']/4,0,:]),'--',color = 'blue',label='3nz/4 phase')
    plt.plot(xgrid,(dens_phase[pars['nz0']/4,0,:]),'--',color = 'red',label='nz/4 phase')
    plt.xlabel(r'$\rho_{tor}$',fontsize=13)
    for i in range(len(qrats)):
        ix = np.argmin(abs(geometry['q']-qrats[i])) 
        plt.axvline(xgrid[ix],color='black')
    ...
    #Extract m's
    #phi_theta = np.zeros((field.nz+4,field.ny,field.nx),dtype = 'complex128')     
    Ep = gradphi[2:-2,:,:] + (omega_complex+(0.0+1.0J)*pars['n0_global']*omegator0[i]*time_ref)*field.apar()[:,:,:]
    Ep_theta = np.zeros((nm,field.ny,field.nx),dtype = 'complex128')     
    Ep_m = np.zeros((nm,field.ny,field.nx),dtype = 'complex128')     

    for i in range(len(xgrid)):
        Ep_theta[:,0,i] = interp(zgrid,Ep[:,0,i],zgridm)
        Ep_theta[:,0,i] = Ep_theta[:,0,i]*np.e**(1J*pars['n0_global']*geometry['q'][i]*np.pi*zgridm)
    for i in range(len(xgrid)):
        Ep_m[:,0,i] = np.fft.fft(Ep_theta[:,0,i])
    if plot_theta:
        for i in range(int(mmin),int(mmax)+1):
            plt.plot(xgrid,np.abs(Ep_m[i,0,:]))
        for i in range(len(qrats)):
            ix = np.argmin(abs(geometry['q']-qrats[i])) 
            plt.axvline(xgrid[ix],color='black')
        plt.xlabel(r'$\rho_{tor}$',fontsize=13)
        plt.ylabel(r'$E_{||m}$',fontsize=13)
        plt.title(r'$E_{||m}$')
        plt.show()
