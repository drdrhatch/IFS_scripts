...
def LN_apar_frequency_nz(suffix,inz,time_step,time_start,time_end,plot=False,pic_path='pic',csv_path='csv',output_csv=False,show=False):
...
def LN_apar_frequency_nz_iky(suffix,inz,iky,time_step,time_start,time_end,plot=False,pic_path='pic',csv_path='csv',output_csv=False,show=False):
    #Where inz is the number of the element in nz
...
def LN_apar_frequency_iky_Z_sum(suffix,min_Z0,max_Z0,Outboard_mid_plane,iky,time_step,time_start,time_end,plot=False,pic_path='pic',csv_path='csv',output_csv=False):
    #Where inz is the number of the element in nz
    ...
    if x_local == False:  #x_local = False
        print("Sorry, cannot handle Global Nonlinear yet...")
        exit
    ...
    for inz in nZ_list:
        ...
        B1_inz_t=B1_inz_t+B1_ky_t_inz[iky,:]*length #weight with dZ
    B1_inz_t=B1_inz_t/sum_length_TEMP	#average
...
def B1_ky_f_spectrum_Z_sum(suffix,iterdb_file_name,time_step,min_Z0=-0.03,max_Z0=0.03,\
...
def B1_ky_f_spectrum_Z_sum_time_set(suffix,iterdb_file_name,\
...
#this one do FFT first and then sum over Z
def B1_ky_f_spectrum_Z_sum_time_set_backup(suffix,iterdb_file_name,\
    min_Z0,max_Z0,Outboard_mid_plane,time_step,time_start,time_end,\
    plot,show,csv_output,pic_path,csv_path):
    #Import the parameters from parameter file using ParIO
    par = Parameters()
...
#def LN_apar_frequency_nz_Doppler(suffix,inz,time_start,time_end,plot=False,pic_path='pic',csv_path='csv',output_csv=False,show=False):
def LN_n1_frequency_nz_iky(suffix,inz,iky,time_step,time_start,time_end,plot=False,pic_path='pic',csv_path='csv',output_csv=False,show=False):
    #Where inz is the number of the element in nz
    ...
    nky0=len(n_list)
    ntime=len(time_list)
    n1_ky_t_inz=np.zeros((nky0,ntime))
    ...
    for i in range(len(time_list)):
        ...
        if x_local:
            ...
            n1_GENE_ky=np.sum(n1_GENE,axis=2) 
        ...
        (nz,nky)=np.shape(n1_ky)
        n1_ky_t_inz[:,i]=n1_ky[inz,:]
    ...
    if plot==True:
        ...
        plt.plot(frequency_kHZ,amplitude_frequency,label='frequency')
        plt.plot(frequency_kHZ,amplitude_growth,label='growth')
        plt.title(r'$n1(/m^3)$'+' at ky_out_board= '+str(ky_GENE_inz[iky]))
        plt.xlabel(r'$f(kHz)$')       
        plt.ylabel(r'$n1(/m^3)$')
        plt.legend()
        plt.savefig(pic_path+'/n1_ky_out_board='+str(ky_GENE_inz[iky])+'.png')


    if output_csv==True:
        d = {'ky_out_board':[ky_GENE_inz[iky]]*len(frequency_kHZ),'frequency(kHZ)':frequency_kHZ,'n1_amplitude_frequency':amplitude_frequency,'B_R(Gauss)amplitude_growth':amplitude_growth}
        df_k=pd.DataFrame(d, columns=['ky_out_board','frequency(kHZ)','B_R(Gauss)amplitude_frequency','n1_amplitude_growth'])
        df_k.to_csv(csv_path+'/n1_inz='+str(inz)+'ky_out_board='+str(ky_GENE_inz[iky])+'.csv',index=False)
    

    return frequency_kHZ,amplitude_frequency,amplitude_growth

def n1_ky_f_spectrum_Z_sum_time_set(suffix,iterdb_file_name,\
    min_Z0,max_Z0,Outboard_mid_plane,time_step,time_start,time_end,\
    plot,show,csv_output,pic_path,csv_path):
    #Import the parameters from parameter file using ParIO
    par = Parameters()
    ...
    inz=int(len(real_Z)/2.)
    frequency_kHZ, amplitude_frequency, amplitude_growth=\
        LN_n1_frequency_nz_iky(suffix,inz,iky,time_step,time_start,time_end,\
        plot=False,pic_path='pic',csv_path='csv',output_csv=False)
    ...
    for i_Z in range(len_nZ):
        ...
        for i_ky in range(nky0):
            amplitude_frequency, amplitude_frequency, amplitude_growth=\
                LN_n1_frequency_nz_iky(suffix,inZ,i_ky,time_step,time_start,time_end,\
                plot=False,pic_path='pic',csv_path='csv',output_csv=False)
    ...
    #****************start of Output***********************
    for i_ky in range(len(n_list)):
        d = {'f(kHz)':frequency_kHZ_ky_sum_Z[i_ky,:],'n1(/m^3)':amplitude_frequency_ky_sum_Z[i_ky,:]}
        df=pd.DataFrame(d, columns=['f(kHz)','n1(/m^3)'])
        df.to_csv(csv_path+'/n1_freq_n_'+str(n_list[i_ky])+'.csv',index=False)
    
        plt.clf()
        plt.plot(frequency_kHZ_ky_sum_Z[i_ky,:],amplitude_frequency_ky_sum_Z[i_ky,:],label='Original')
        plt.xlabel('frequency (kHz)')
        plt.ylabel(r'$n_1(10^{19}/m^3)$') 
        plt.title(r'$n_1(10^{19}/m^3)$ spectrogram of n='+str(n_list[i_ky]),fontsize=18)
        plt.savefig(pic_path+'/n1_freq_n_'+str(n_list[i_ky])+'.png')
        ...
        plt.ylabel(r'$n_1(10^{19}/m^3)$')  
        plt.title(r'$n_1(10^{19}/m^3)$ spectrogram of n='+str(n_list[i_ky]),fontsize=18)
        plt.savefig(pic_path+'/Interp_n1_freq_n_'+str(n_list[i_ky])+'.png')
    ...
    #**********end of Sum over ky*********************
    if csv_output==True:
        d = {'f(kHz)':uni_freq,'n1(/m^3)':amplitude_frequency_uni_ky_sum}
        df=pd.DataFrame(d, columns=['f(kHz)','n1(/m^3)'])
        df.to_csv('0n1_from_'+str(max_Z0)+'_to_'+str(min_Z0)+'.csv',index=False)

    if plot==True:
        plt.clf()
        plt.xlabel(r'$Frequency(kHz)$',fontsize=15)
        plt.ylabel(r'$n_1(/m^3)$',fontsize=15)
        plt.plot(uni_freq,amplitude_frequency_uni_ky_sum,label='Inpter')
        #plt.legend()
        plt.title(r'$n_1(/m^3)$ spectrogram',fontsize=18)
        plt.savefig('0n1_RIP_freq_spectrogram.png')
    ...
    if plot==True: 
        ...
        plt.ylabel(r'$n_1(/m^3)$',fontsize=15)
        plt.title(r'$n_1(/m^3)$ spectrogram',fontsize=18)
        plt.savefig('0n_1RIP_freq_spectrogram.png')
        if show==True:
            plt.show()
