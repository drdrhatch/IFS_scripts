...
def RIP_f_spectrum_density(suffix,iterdb_file_name,manual_Doppler,min_Z0,max_Z0,\
    ...
    for i in range(len(time_list)):
        ...
        if x_local:
            ...
            apar=field.apar()[:,:,:]+0.j
            apar_ky=np.sum(apar[:,:,:]**2.,axis=2)
            #print('apar_ky'+str(apar_ky)) #still complex
            B1_GENE_ky0=apar_ky*(ky_GENE_grid*Apar_to_B1)**2.
    ...
    for iky in range(nky0):
        #B1_inz_t=(B1_ky_t[iky,:]+0.j)**0.5
        if iky==0:
            B1_inz_t=(B1_ky_t[iky,:])**0.5
        else:
            B1_inz_t=(2.*B1_ky_t[iky,:])**0.5
...
def RIP_freq_IDL(suffix,iterdb_file_name,manual_Doppler,min_Z0,max_Z0,\
    Outboard_mid_plane,time_step,time_start,time_end,window_for_FFT,\
    plot,show,csv_output,pic_path,csv_path):
    #Where inz is the number of the element in nz
    ...
    for i in range(len(time_list)):
        ...
        if x_local:
            ...
            #print('apar_ky'+str(apar_ky)) #still complex
            B1_GENE_ky0=apar_ky*(ky_GENE_grid*Apar_to_B1)
            B1_GENE_ky= np.zeros(np.shape(B1_GENE_ky0[0,:]),dtype=complex)
    ...
    for iky in range(nky0):
        ...
        if iky==0:
            ...
            B1_inz_t=(2.*B1_ky_t[iky,:])
        print('B1_ky_t[iky,:]'+str(B1_ky_t[iky,:]))
        print('B1_inz_t'+str(B1_inz_t))
        #frequency,amplitude_frequency,amplitude_growth=window_FFT_function_time(B1_inz_t,time_list,plot=False)
        log_B=np.log(B1_inz_t)
        dt_mean=np.mean(abs(time_list[1:]-time_list[:-1]))
        omega_TEMP=np.imag(log_B[1:]-log_B[:-1])/dt_mean
        omega=np.median(omega_TEMP)
        omega_list.append(omega)
        amplitude_list.append(np.mean(B1_inz_t))
        '''
        frequency,amplitude_frequency_sq=spectral_density(B1_inz_t,time_list,window_for_FFT=window_for_FFT,plot=False)
        frequency_kHZ=frequency
        amplitude_frequency=abs(np.sqrt(amplitude_frequency_sq))
    ...
    #f,amplitude_f=k_f_density_plot(f_ky_f,B1_ky_f,ky_list,n_list,pic_path,csv_path,name='B1')
    f=np.array(omega_list)
    amplitude_f=np.array(amplitude_list)
    return f,amplitude_f
