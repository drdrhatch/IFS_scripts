...
def kz_from_dfielddz_bessel(kperp, zgrid, jacobian, field, plot, name, zstart = 0., zend = 0.):
    alpha = 2./3.
    bessel_factor = 1. / np.sqrt(1. + 2. * (kperp**2 + np.pi * alpha * kperp**4) /
                    (1. + alpha * kperp**2))
    if 1 == 0:
...
#        print('phi = ', phi[0])
        ...
        if not central:
            ave_kz, zstart, zend = kz_from_dfielddz_bessel(kperp, zgrid,jacobian, phi, True, 'phi')
        else:
            ave_kz, zstart, zend = kz_from_dfielddz_bessel(kperp, zgrid,jacobian, phi, False, 'phi', zstart = -.95, zend = .95)
        ...
        ave_kz_smoothed01, zstart01, zend01  = kz_from_dfielddz_bessel(kperp, zgrid, jacobian, phi_smoothed01, False, 'phi smoothed once', zstart = zstart, zend = zend)
        ...
        ave_kz_smoothed02, zstart02, zend02  = kz_from_dfielddz_bessel(kperp, zgrid, jacobian, phi_smoothed02, False, 'phi smoothed twice', zstart = zstart, zend = zend)
        ...
        f = open('averaged3' + suffix + str(gauss).replace(".",""), 'w')
        sys.stdout = f
        print ''
        print ''
    ...
    if 1 == 0:
        ...
        ave_kz = kz_from_dfielddz_bessle(kperp, zgrid,jacobian, phi, True, 'phi')
        f = open('averaged_3_' + suffix + str(gauss).replace(".",""), 'w')
        sys.stdout = f
        if gauss == 0.:
