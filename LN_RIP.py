...
from LN_tools import RIP_f_spectrum
...
for i_Z_list in range(len(Z_list)):
    ...
    f_ky_f,B1_ky_f,growth_ky_f=\
        RIP_f_spectrum(suffix,iterdb_file_name,\
    ...
    if frequency_all==True:
        B1=np.average(abs(B1_ky_f))/2.  #double count
    else:
        ...
        for iky in range(n_ky):
            for i_f in range(n_f):
                if frequency_min<=f_ky_f[iky][i_f] and f_ky_f[iky][i_f]<=frequency_max:
                    ...
                    B1_RIP0=B1_RIP0+abs(B1_ky_f[iky][i_f])*abs(f_ky_f[iky][i_f]-f_ky_f[iky][i_f-1])
        B1=B1_RIP0/int_f
