...
for i in range(nt):
    ...
    wi[i] = fdict['wi']
    ne[i,:] = fdict['ne'][1,:]
    omne[i,:] = -fd_d1_o4_smoothend(ne[i,:],rhot)
    ni[i,:] = fdict['ni2'][1,:]
    omni[i,:] = -fd_d1_o4_smoothend(ni[i,:],rhot)
    nz[i,:] = fdict['nz'][1,:]
    omnz[i,:] = -fd_d1_o4_smoothend(nz[i,:],rhot)
    te[i,:] = fdict['te'][1,:]
    omte[i,:] = -fd_d1_o4_smoothend(te[i,:],rhot)
    ti[i,:] = fdict['ti'][1,:]
    omti[i,:] = -fd_d1_o4_smoothend(ti[i,:],rhot)
...
if output_gene:
    zeros = np.zeros(len(ne[-1,:]))
    f=open('gene_profiles_e','w')
    f.write('# 1.rho_tor 2.zeros 3.Te(kev) 4.ne(10^19m^-3)\n#\n')
    np.savetxt(f,np.column_stack((rhot,zeros,te[-1,:],ne[-1,:])))
    f.close()
    f=open('gene_profiles_i','w')
    f.write('# 1.rho_tor 2.zeros 3.Ti(kev) 4.ni(10^19m^-3)\n#\n')
    np.savetxt(f,np.column_stack((rhot,zeros,ti[-1,:],ni[-1,:])))
    f.close()
    f=open('gene_profiles_c','w')
    f.write('# 1.rho_tor 2.zeros 3.TC(kev) 4.nC(10^19m^-3)\n#\n')
    np.savetxt(f,np.column_stack((rhot,zeros,ti[-1,:],nz[-1,:])))
    f.close()
    plt.plot(rhot,ne[-1,:])
...
plt.ylabel(r'$Q_e(kWm^{-3})$',size=14)
plt.legend()
plt.xlabel(r'$\rho_{tor}$',size=14)
...
plt.ylabel(r'$Q_i(kWm^{-3})$',size=14)
plt.legend()
plt.xlabel(r'$\rho_{tor}$',size=14)
