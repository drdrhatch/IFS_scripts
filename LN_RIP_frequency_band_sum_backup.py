...
frequency_max0=100.       #maximum frequency(Lab Frame) to sum over in kHz
frequency_min0=0.       #minimum frequency(Lab Frame) in sum over in kHz

EXB_on=True             #Switch to True is the simulation has the EXB shear
    ...
    for nZ in range(len(real_Z)):
        ...
        if Z_list[nZ_list]-Delta_Z/2.<Z and Z<=Z_list[nZ_list]+Delta_Z/2.:
            ...
            if frequency_all==True:
                B1=np.average(amplitude_frequency_sum)/2.  #double count
            else:
